...
```{r removing 0 and 500ng titrations}
...
```{r overlapping seq sanity check }
...
```{r seq overlap comparison, fig.width = 12}
...
```{r filtering 100, echo=FALSE, message=FALSE, results='hide'}

fil_100_1 <- titra_list[[2]] %>% #subset from the list that stores all the titrations
  rownames_to_column(var = "miRNA") %>% 
  mutate(Sum = rowSums(.[2:4])) %>%  #removing sequences with less than 1 read count across all samples
  filter(Sum >= 1) %>% 
  column_to_rownames(var = "miRNA") %>% 
...
fil_100_2 <- titra_list[[2]] %>% #keeping sequences with counts of atleast 1 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 1))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n1 count\nin all\nsamples")
...
fil_100_3 <- titra_list[[2]] %>% #keeping sequences with counts of atleast 2 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 2))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n2 counts")
...
fil_100_4 <- titra_list[[2]] %>% #keeping sequences with counts of atleast 3 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 3))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n3 counts")
...
fil_100 <- list(fil_100_1, fil_100_2, fil_100_3, fil_100_4)

fil_100 <- lapply(fil_100, f1) #lapply = apply for list, f = function describing the dplyr steps

fil_100 <- lapply(fil_100, function(x) mutate(x, titration = "100ng"))

```

```{r filtering 50, echo=FALSE, message=FALSE, results='hide'}

fil_50_1 <- titra_list[[3]] %>% #subset from the list that stores all the titrations
  rownames_to_column(var = "miRNA") %>% 
  mutate(Sum = rowSums(.[2:4])) %>%  #removing sequences with less than 1 read count across all samples
  filter(Sum >= 1) %>% 
  column_to_rownames(var = "miRNA") %>% 
...
fil_50_2 <- titra_list[[3]] %>% #keeping sequences with counts of atleast 1 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 1))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n1 count\nin all\nsamples")
...
fil_50_3 <- titra_list[[3]] %>% #keeping sequences with counts of atleast 2 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 2))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n2 counts")
...
fil_50_4 <- titra_list[[3]] %>% #keeping sequences with counts of atleast 3 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 3))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n3 counts")
...
fil_50 <- list(fil_50_1, fil_50_2, fil_50_3, fil_50_4)

fil_50 <- lapply(fil_50, f1) #lapply = apply for list, f = function describing the dplyr steps

fil_50 <- lapply(fil_50, function(x) mutate(x, titration = "50ng"))

```

```{r filtering 25, echo=FALSE,  message=FALSE, results='hide'}

fil_25_1 <- titra_list[[4]] %>% #subset from the list that stores all the titrations
  rownames_to_column(var = "miRNA") %>% 
  mutate(Sum = rowSums(.[2:4])) %>%  #removing sequences with less than 1 read count across all samples
  filter(Sum >= 1) %>% 
  column_to_rownames(var = "miRNA") %>% 
...
fil_25_2 <- titra_list[[4]] %>% #keeping sequences with counts of atleast 1 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 1))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n1 count\nin all\nsamples")
...
fil_25_3 <- titra_list[[4]] %>% #keeping sequences with counts of atleast 2 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 2))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n2 counts")
...
fil_25_4 <- titra_list[[4]] %>% #keeping sequences with counts of atleast 3 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 3))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n3 counts")
...
fil_25 <- list(fil_25_1, fil_25_2, fil_25_3, fil_25_4)

fil_25 <- lapply(fil_25, f1) #lapply = apply for list, f = function describing the dplyr steps

fil_25 <- lapply(fil_25, function(x) mutate(x, titration = "25ng"))

```    

```{r filtering 5, echo=FALSE, message=FALSE, results='hide'}

fil_5_1 <- titra_list[[5]] %>% #subset from the list that stores all the titrations
  rownames_to_column(var = "miRNA") %>% 
  mutate(Sum = rowSums(.[2:4])) %>%  #removing sequences with less than 1 read count across all samples
  filter(Sum >= 1) %>% 
  column_to_rownames(var = "miRNA") %>% 
...
fil_5_2 <- titra_list[[5]] %>% #keeping sequences with counts of atleast 1 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 1))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n1 count\nin all\nsamples")
...
fil_5_3 <- titra_list[[5]] %>% #keeping sequences with counts of atleast 2 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 2))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n2 counts")
...
fil_5_4 <- titra_list[[5]] %>% #keeping sequences with counts of atleast 3 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 3))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n3 counts")
...
fil_5 <- list(fil_5_1, fil_5_2, fil_5_3, fil_5_4)

fil_5 <- lapply(fil_5, f1) #lapply = apply for list, f = function describing the dplyr steps

fil_5 <- lapply(fil_5, function(x) mutate(x, titration = "5ng"))

``` 

```{r filtering 1, echo=FALSE, message=FALSE, results='hide'}

fil_1_1 <- titra_list[[6]] %>% #subset from the list that stores all the titrations
  rownames_to_column(var = "miRNA") %>% 
  mutate(Sum = rowSums(.[2:4])) %>%  #removing sequences with less than 1 read count across all samples
  filter(Sum >= 1) %>% 
  column_to_rownames(var = "miRNA") %>% 
...
fil_1_2 <- titra_list[[6]] %>% #keeping sequences with counts of atleast 1 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 1))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n1 count\nin all\nsamples")
...
fil_1_3 <- titra_list[[6]] %>% #keeping sequences with counts of atleast 2 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 2))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n2 counts")
...
fil_1_4 <- titra_list[[6]] %>% #keeping sequences with counts of atleast 3 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 3))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n3 counts")
...
fil_1 <- list(fil_1_1, fil_1_2, fil_1_3, fil_1_4)

fil_1 <- lapply(fil_1, f1) #lapply = apply for list, f = function describing the dplyr steps

fil_1 <- lapply(fil_1, function(x) mutate(x, titration = "1ng"))

``` 

```{r extra code, include=FALSE, echo=FALSE, eval=FALSE}

cols <- colSums(fil_250_1) #summing to get total number of reads
cols <- apply(fil_250_1, 2, sum)

fil_250_1 <- rbind(fil_250_1, cols)

fil_250_1 <- fil_250_1 %>% 
   dplyr::arrange(desc(m19833))

row.names(fil_250_1) [1] <- "Total_Reads"

attributes(fil_250_1) <- NULL

#(1) and (2) perform the same fucntion
list_250 <- lapply(list_250, function(x) rownames_to_column(x, var = "miRNA")) #using lapply (1)
list_250 <- map(list_250, ~adorn_totals(.x, "row")) #using map from purrr (2)
...
```{r making df of total seq by filtering, fig.width = 12}
...
```{r extracting first row of lists}
...
```{r making df with total RPM for all filters + tirations, fig.width = 12}
...
```{r redoing graph 2, echo=FALSE, message=FALSE, fig.width = 14, fig.height = 9, , results='hide'}

list_250 <- eDat2 [c(1, 7)] 
list_100 <- eDat2 [c(2, 8)] 
list_50 <- eDat2 [c(3, 9)] 
list_25 <- eDat2 [c(4, 10)]
list_5 <- eDat2 [c(5, 11)] 
list_1 <- eDat2 [c(6, 12)] 

robsamp_list <- list (list_250, list_100, list_50, list_25, list_5, list_1)


fil_seq_1 <- fil_seq

#250
fil_250_1 <- robsamp_list[[1]] %>% #subset from the list that stores all the titrations
  rownames_to_column(var = "miRNA") %>% 
  mutate(Sum = rowSums(.[2:3])) %>%  #removing sequences with less than 1 read count across all samples
  filter(Sum >= 1) %>% 
  column_to_rownames(var = "miRNA") %>% 
  dplyr::select(-Sum)  %>% 
  mutate(filter = "at least\n1 count in\n1 sample")
...
fil_250_2 <- robsamp_list[[1]] %>% #keeping sequences with counts of atleast 1 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 1))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n1 count\nin all\nsamples")
...
fil_250_3 <- robsamp_list[[1]] %>% #keeping sequences with counts of atleast 2 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 2))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n2 counts")
...
fil_250_4 <- robsamp_list[[1]] %>% #keeping sequences with counts of atleast 3 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 3))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n3 counts")
...
fil_100_1 <- robsamp_list[[2]] %>% #subset from the list that stores all the titrations
  rownames_to_column(var = "miRNA") %>% 
  mutate(Sum = rowSums(.[2:3])) %>%  #removing sequences with less than 1 read count across all samples
  filter(Sum >= 1) %>% 
  column_to_rownames(var = "miRNA") %>% 
...
fil_100_2 <- robsamp_list[[2]] %>% #keeping sequences with counts of atleast 1 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 1))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n1 count\nin all\nsamples")
...
fil_100_3 <- robsamp_list[[2]] %>% #keeping sequences with counts of atleast 2 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 2))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n2 counts")
...
fil_100_4 <- robsamp_list[[2]] %>% #keeping sequences with counts of atleast 3 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 3))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n3 counts")
...
fil_50_1 <- robsamp_list[[3]] %>% #subset from the list that stores all the titrations
  rownames_to_column(var = "miRNA") %>% 
  mutate(Sum = rowSums(.[2:3])) %>%  #removing sequences with less than 1 read count across all samples
  filter(Sum >= 1) %>% 
  column_to_rownames(var = "miRNA") %>% 
...
fil_50_2 <- robsamp_list[[3]] %>% #keeping sequences with counts of atleast 1 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 1))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n1 count\nin all\nsamples")
...
fil_50_3 <- robsamp_list[[3]] %>% #keeping sequences with counts of atleast 2 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 2))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n2 counts")
...
fil_50_4 <- robsamp_list[[3]] %>% #keeping sequences with counts of atleast 3 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 3))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n3 counts")
...
fil_25_1 <- robsamp_list[[4]] %>% #subset from the list that stores all the titrations
  rownames_to_column(var = "miRNA") %>% 
  mutate(Sum = rowSums(.[2:3])) %>%  #removing sequences with less than 1 read count across all samples
  filter(Sum >= 1) %>% 
  column_to_rownames(var = "miRNA") %>% 
...
fil_25_2 <- robsamp_list[[4]] %>% #keeping sequences with counts of atleast 1 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 1))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n1 count\nin all\nsamples")
...
fil_25_3 <- robsamp_list[[4]] %>% #keeping sequences with counts of atleast 2 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 2))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n2 counts")
...
fil_25_4 <- robsamp_list[[4]] %>% #keeping sequences with counts of atleast 3 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 3))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n3 counts")
...
fil_5_1 <- robsamp_list[[5]] %>% #subset from the list that stores all the titrations
  rownames_to_column(var = "miRNA") %>% 
  mutate(Sum = rowSums(.[2:3])) %>%  #removing sequences with less than 1 read count across all samples
  filter(Sum >= 1) %>% 
  column_to_rownames(var = "miRNA") %>% 
...
fil_5_2 <- robsamp_list[[5]] %>% #keeping sequences with counts of atleast 1 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 1))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n1 count\nin all\nsamples")
...
fil_5_3 <- robsamp_list[[5]] %>% #keeping sequences with counts of atleast 2 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 2))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n2 counts")
...
fil_5_4 <- robsamp_list[[5]] %>% #keeping sequences with counts of atleast 3 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 3))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n3 counts")
...
fil_1_1 <- robsamp_list[[6]] %>% #subset from the list that stores all the titrations
  rownames_to_column(var = "miRNA") %>% 
  mutate(Sum = rowSums(.[2:3])) %>%  #removing sequences with less than 1 read count across all samples
  filter(Sum >= 1) %>% 
  column_to_rownames(var = "miRNA") %>% 
...
fil_1_2 <- robsamp_list[[6]] %>% #keeping sequences with counts of atleast 1 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 1))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n1 count\nin all\nsamples")
...
fil_1_3 <- robsamp_list[[6]] %>% #keeping sequences with counts of atleast 2 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 2))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n2 counts")
...
fil_1_4 <- robsamp_list[[6]] %>% #keeping sequences with counts of atleast 3 in all of the samples
  rownames_to_column(var = "miRNA") %>% 
  filter_all(all_vars(. >= 3))  %>% 
  column_to_rownames(var = "miRNA") %>% 
  mutate(filter = "at least\n3 counts")
...
#fil_seq$Total_Sequences <- as.numeric(c("11930", "5448", "2257", "1276", 
#                                        "10819", "4639", "1837", "1005", 
#                                        "12219", "5771", "2326", "1258", 
#                                        "11671", "5162", "2127", "1166", 
#                                        "12884", "6040" , "2686", "1550", 
#                                        "6549", "1837", "724", "401"))
...
g2 <- fil_seq %>% 
  ...
  geom_text(aes(label = Total_Sequences), vjust = -0.8, colour = "#333333", position = position_dodge(0.65)) +
  scale_fill_manual(values = col_filters) +
  labs(title = "Comparison of Total No of Sequences - Only Rob Lab Samples", subtitle = "by 4 Filtering Criteria") +
  scale_y_continuous(labels = comma, expand = c(0, 0)) + #to make yaxis units as whole numbers rather than in exponential form
  my_theme +
  coord_cartesian(y = c(0, 2000)) +
  theme(legend.position = "bottom")
...
```{r, filtering by percentage of samples}
...
```{r zero counts removal, message=FALSE, results='hide'}
...
```{r min count 2 and 3, results="hide"}
...
```{r 10 percent, message=FALSE, results='hide'}
...
```{r 20 percent, message=FALSE, results='hide'}
...
```{r 30 percent, message=FALSE, results='hide'}
...
```{r 40 percent, message=FALSE, results='hide'}
...
```{r 50 percent, message=FALSE, results='hide'}
...
```{r 60 percent, message=FALSE, results='hide'}
...
```{r 70 percent, message=FALSE, results='hide'}
...
```{r 80 percent, message=FALSE, results='hide'}
...
```{r pl290 perc filter, message=FALSE, results='hide'}
...
```{r tcr perc filter, message=FALSE, results='hide'}
...
```{r con perc filter, message=FALSE, results='hide'}
...
```{r s1 min2 min3, cache=TRUE, echo=FALSE}

head(eDat_min_2)

s1_min_2 <- eDat_min_2[c(1:6)]
...
s1_min_2 <- s1_min_2 %>% 
  filter(!more_than_2 == 0)

#in X% of samples

s1_perc_2 <- s1_min_2 %>% 
  filter(more_than_2 == 1)
nrow(s1_perc_2)
...
s1_perc_6 <- s1_min_2 %>% 
  filter(more_than_2 == 5)
nrow(s1_perc_6) 

s1_perc_all <- s1_min_2 %>% 
  filter(more_than_2 == 6)
nrow(s1_perc_all) 


s1_min2_perc <- as.data.frame(nrow(s1_perc_2))
s1_min2_perc$perc_3 <- nrow(s1_perc_3)
s1_min2_perc$perc_4 <- nrow(s1_perc_4)
s1_min2_perc$perc_5 <- nrow(s1_perc_5)
s1_min2_perc$perc_6 <- nrow(s1_perc_6)
s1_min2_perc$all <- nrow(s1_perc_all)

colnames(s1_min2_perc)[1] <- "At least\n1 count in\n1 sample"

s1_min2_perc <- as.data.frame(t(s1_min2_perc))

colnames(s1_min2_perc)[1] <- "Total_Sequences"

s1_min2_perc <- s1_min2_perc %>% 
  rownames_to_column(var = "Percent_of_samples")

s1_min2_perc$Sample <- "Rob Lab T1"
...
s1_min2_all <- as.data.frame(nrow(s1_perc_1))
s1_min2_all$perc_2 <- nrow(s1_perc_2)
s1_min2_all$perc_3 <- nrow(s1_perc_3)
s1_min2_all$perc_4 <- nrow(s1_perc_4)
s1_min2_all$perc_5 <- nrow(s1_perc_5)
s1_min2_all$perc_6 <- nrow(s1_perc_6)

s1_min2_all <- as.data.frame(t(s1_min2_all))

s1_min2_all <- s1_min2_all %>% 
  rownames_to_column(var = "Percent_of_samples")

colnames(s1_min2_all)[2] <- "Total_Sequences"

s1_min2_all$Sample <- "Rob Lab T1"



#For min_3

s1_min_3 <- eDat_min_3[c(1:6)]

s1_min_3 <- s1_min_3 %>% 
  rowwise() %>%
  do((.) %>% 
  as.data.frame %>% 
  mutate(more_than_3 = sum(. >= 3)))

s1_min_3 <- s1_min_3 %>% 
  filter(!more_than_3 == 0)

#in X% of samples

s1_perc_2 <- s1_min_3 %>% 
  filter(more_than_3 == 1)
nrow(s1_perc_2) 


s1_perc_3 <- s1_min_3 %>% 
  filter(more_than_3 == 2)
nrow(s1_perc_3) 


s1_perc_4 <- s1_min_3 %>% 
  filter(more_than_3 == 3)
nrow(s1_perc_4) 
...
s1_perc_6 <- s1_min_3 %>% 
  filter(more_than_3 == 5)
nrow(s1_perc_6) 

s1_perc_all <- s1_min_3 %>% 
  filter(more_than_3 == 6)
nrow(s1_perc_all) 


s1_min3_perc <- as.data.frame(nrow(s1_perc_2))
s1_min3_perc$perc_3 <- nrow(s1_perc_3)
s1_min3_perc$perc_4 <- nrow(s1_perc_4)
s1_min3_perc$perc_5 <- nrow(s1_perc_5)
s1_min3_perc$perc_6 <- nrow(s1_perc_6)
s1_min3_perc$all <- nrow(s1_perc_all)

colnames(s1_min3_perc)[1] <- "At least\n1 count in\n1 sample"

s1_min3_perc <- as.data.frame(t(s1_min3_perc))

colnames(s1_min3_perc)[1] <- "Total_Sequences"

s1_min3_perc <- s1_min3_perc %>% 
  rownames_to_column(var = "Percent_of_samples")

s1_min3_perc$Sample <- "Rob Lab T1"
...
s1_min3_all <- as.data.frame(nrow(s1_perc_1))
s1_min3_all$perc_2 <- nrow(s1_perc_3)
s1_min3_all$perc_3 <- nrow(s1_perc_3)
s1_min3_all$perc_4 <- nrow(s1_perc_4)
s1_min3_all$perc_5 <- nrow(s1_perc_5)
s1_min3_all$perc_6 <- nrow(s1_perc_6)

s1_min3_all <- as.data.frame(t(s1_min3_all))

s1_min3_all <- s1_min3_all %>% 
  rownames_to_column(var = "Percent_of_samples")

colnames(s1_min3_all)[2] <- "Total_Sequences"

s1_min3_all$Sample <- "Rob Lab T1"

```  

```{r s2 min2 min3, cache=TRUE, echo=FALSE}

head(eDat_min_2)

s2_min_2 <- eDat_min_2[c(7:12)]
...
s2_perc_all <- s2_min_2 %>% 
  filter(more_than_2 == 6)
nrow(s2_perc_all) 


s2_min2_perc <- as.data.frame(nrow(s2_perc_2))
s2_min2_perc$perc_3 <- nrow(s2_perc_3)
s2_min2_perc$perc_4 <- nrow(s2_perc_4)
s2_min2_perc$perc_5 <- nrow(s2_perc_5)
s2_min2_perc$perc_6 <- nrow(s2_perc_6)
s2_min2_perc$all <- nrow(s2_perc_all)

colnames(s2_min2_perc)[1] <- "At least\n1 count in\n1 sample"

s2_min2_perc <- as.data.frame(t(s2_min2_perc))

colnames(s2_min2_perc)[1] <- "Total_Sequences"

s2_min2_perc <- s2_min2_perc %>% 
  rownames_to_column(var = "Percent_of_samples")

s2_min2_perc$Sample <- "Rob Lab T2"
...
s2_min2_all <- as.data.frame(nrow(s2_perc_1))
s2_min2_all$perc_2 <- nrow(s2_perc_2)
s2_min2_all$perc_3 <- nrow(s2_perc_3)
s2_min2_all$perc_4 <- nrow(s2_perc_4)
s2_min2_all$perc_5 <- nrow(s2_perc_5)
s2_min2_all$perc_6 <- nrow(s2_perc_6)

s2_min2_all <- as.data.frame(t(s2_min2_all))

s2_min2_all <- s2_min2_all %>% 
  rownames_to_column(var = "Percent_of_samples")

colnames(s2_min2_all)[2] <- "Total_Sequences"

s2_min2_all$Sample <- "Rob Lab T2"



#For min_3

s2_min_3 <- eDat_min_3[c(7:12)]
...
s2_perc_all <- s2_min_3 %>% 
  filter(more_than_3 == 6)
nrow(s2_perc_all) 


s2_min3_perc <- as.data.frame(nrow(s2_perc_2))
s2_min3_perc$perc_3 <- nrow(s2_perc_3)
s2_min3_perc$perc_4 <- nrow(s2_perc_4)
s2_min3_perc$perc_5 <- nrow(s2_perc_5)
s2_min3_perc$perc_6 <- nrow(s2_perc_6)
s2_min3_perc$all <- nrow(s2_perc_all)

colnames(s2_min3_perc)[1] <- "At least\n1 count in\n1 sample"

s2_min3_perc <- as.data.frame(t(s2_min3_perc))

colnames(s2_min3_perc)[1] <- "Total_Sequences"

s2_min3_perc <- s2_min3_perc %>% 
  rownames_to_column(var = "Percent_of_samples")

s2_min3_perc$Sample <- "Rob Lab T2"
...
s2_min3_all <- as.data.frame(nrow(s2_perc_1))
s2_min3_all$perc_2 <- nrow(s2_perc_2)
s2_min3_all$perc_3 <- nrow(s2_perc_3)
s2_min3_all$perc_4 <- nrow(s2_perc_4)
s2_min3_all$perc_5 <- nrow(s2_perc_5)
s2_min3_all$perc_6 <- nrow(s2_perc_6)

s2_min3_all <- as.data.frame(t(s2_min3_all))

s2_min3_all <- s2_min3_all %>% 
  rownames_to_column(var = "Percent_of_samples")

colnames(s2_min3_all)[2] <- "Total_Sequences"

s2_min3_all$Sample <- "Rob Lab T2"

```  

```{r con min2 min3, cache=TRUE, echo=FALSE}

head(eDat_min_2)

con_min_2 <- eDat_min_2[c(13:18)]
...
con_perc_2 <- con_min_2 %>% 
  filter(more_than_2 == 1)
nrow(con_perc_2) 


con_perc_3 <- con_min_2 %>% 
  filter(more_than_2 == 2)
nrow(con_perc_3) 


con_perc_4 <- con_min_2 %>% 
  filter(more_than_2 == 3)
nrow(con_perc_4)


con_perc_5 <- con_min_2 %>% 
  filter(more_than_2 == 4)
nrow(con_perc_5) 

con_perc_6 <- con_min_2 %>% 
  filter(more_than_2 == 5)
nrow(con_perc_6) 

con_perc_all <- con_min_2 %>% 
  filter(more_than_2 == 6)
nrow(con_perc_all) 


con_min2_perc <- as.data.frame(nrow(con_perc_2))
con_min2_perc$perc_3 <- nrow(con_perc_3)
con_min2_perc$perc_4 <- nrow(con_perc_4)
con_min2_perc$perc_5 <- nrow(con_perc_5)
con_min2_perc$perc_6 <- nrow(con_perc_6)
con_min2_perc$all <- nrow(con_perc_all)

colnames(con_min2_perc)[1] <- "At least\n1 count in\n1 sample"

con_min2_perc <- as.data.frame(t(con_min2_perc))

colnames(con_min2_perc)[1] <- "Total_Sequences"

con_min2_perc <- con_min2_perc %>% 
  rownames_to_column(var = "Percent_of_samples")

con_min2_perc$Sample <- "GSC Control"


#In more than x% of samples

con_perc_1 <- con_min_2 %>% 
  filter(more_than_2 == 6)
...
con_min2_all <- as.data.frame(nrow(con_perc_1))
con_min2_all$perc_2 <- nrow(con_perc_2)
con_min2_all$perc_3 <- nrow(con_perc_3)
con_min2_all$perc_4 <- nrow(con_perc_4)
con_min2_all$perc_5 <- nrow(con_perc_5)
con_min2_all$perc_6 <- nrow(con_perc_6)

con_min2_all <- as.data.frame(t(con_min2_all))

con_min2_all <- con_min2_all %>% 
  rownames_to_column(var = "Percent_of_samples")

colnames(con_min2_all)[2] <- "Total_Sequences"

con_min2_all$Sample <- "GSC Control"



#For min_3

con_min_3 <- eDat_min_3[c(13:18)]
...
con_perc_2 <- con_min_3 %>% 
  filter(more_than_3 == 1)
nrow(con_perc_2) 


con_perc_3 <- con_min_3 %>% 
  filter(more_than_3 == 2)
nrow(con_perc_3) 


con_perc_4 <- con_min_3 %>% 
  filter(more_than_3 == 3)
nrow(con_perc_4) 


con_perc_5 <- con_min_3 %>% 
  filter(more_than_3 == 4)
nrow(con_perc_5) 

con_perc_6 <- con_min_3 %>% 
  filter(more_than_3 == 5)
nrow(con_perc_6) 

con_perc_all <- con_min_3 %>% 
  filter(more_than_3 == 6)
nrow(con_perc_all) 


con_min3_perc <- as.data.frame(nrow(con_perc_2))
con_min3_perc$perc_3 <- nrow(con_perc_3)
con_min3_perc$perc_4 <- nrow(con_perc_4)
con_min3_perc$perc_5 <- nrow(con_perc_5)
con_min3_perc$perc_6 <- nrow(con_perc_6)
con_min3_perc$all <- nrow(con_perc_all)

colnames(con_min3_perc)[1] <- "At least\n1 count in\n1 sample"

con_min3_perc <- as.data.frame(t(con_min3_perc))

colnames(con_min3_perc)[1] <- "Total_Sequences"

con_min3_perc <- con_min3_perc %>% 
  rownames_to_column(var = "Percent_of_samples")

con_min3_perc$Sample <- "GSC Control"


#In more than x% of samples

con_perc_1 <- con_min_3 %>% 
  filter(more_than_3 == 5)
...
con_min3_all <- as.data.frame(nrow(con_perc_1))
con_min3_all$perc_2 <- nrow(con_perc_2)
con_min3_all$perc_3 <- nrow(con_perc_3)
con_min3_all$perc_4 <- nrow(con_perc_4)
con_min3_all$perc_5 <- nrow(con_perc_5)
con_min3_all$perc_6 <- nrow(con_perc_6)

con_min3_all <- as.data.frame(t(con_min3_all))

con_min3_all <- con_min3_all %>% 
  rownames_to_column(var = "Percent_of_samples")

colnames(con_min3_all)[2] <- "Total_Sequences"

con_min3_all$Sample <- "GSC COntrol"
...
```{r percent plot, fig.width = 20, fig.height=5}
...
```{r min 2 percent plot, fig.width = 20, fig.height=5}
...
```{r min 3 percent plot, fig.width = 20, fig.height=5}
...
```{r 576 seqs exp, fig.width = 25, fig.height=15}
...
```{r 0ng exps plot 2, fig.width = 25, fig.height=15}
...
```{r 0ng exps plot 3, fig.width = 20, fig.height=25}
